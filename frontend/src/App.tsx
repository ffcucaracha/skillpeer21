...
function ProfileEditor({ user, onDone }: { user: User; onDone: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => api.updateProfile({ telegram_username: telegram.trim() || null, telegram_visibility: visibility }),
    onSuccess: async () => {
      await Promise.all([
...
function DashboardPage({ onLogout }: { onLogout: () => void }) {
  ...
  const removeSkillMutation = useMutation({
    mutationFn: (skill: UserSkill) => api.removeSkillIntent(skill.skill_id, skill.intent),
    onSuccess: async () => {
      await Promise.all([
